...
async function scrapeWithPuppeteer(): Promise<PuppeteerResult> {
    ...
    // Browser | null -> browser 타입 또는 null을 허용 하고
    // = null -> 초기 값은 null로 설정 한다는 의미
    ...
    try {
        ...
        await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebkit/537.36 (KHTML, like Gecko) Chrome/910.0.39 Safari/537.36');
                ...
                productCards.forEach(card => {
                    ...
                    if(nameElement && priceElement) {
                        productList.push({
                            ...
                            price: Number(priceElement.textContent?.trim() || ''),
                            image: imageElement?.src || ''
                            // ?는 앞의 요소가 있으면 .뒤의 기능을 실행하고 없으면 ''-> 널값을 대입하라는 의미
                        });
                    }
                });
            });
        ...
        // // 무한 스크롤(스크롤을 내릴때 새로운 데이터가 로드되는 페이지) 추출
        ...
        try {
            ...
            console.log(' 리소스 정리 중 오류 발생: ', cleanupMessage);
        }
    }
}
...
export async function GET() {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : '알 수 없는 오류';
        console.log('API 라우터 오류: ', errorMessage);
        return NextResponse.json(
            {
            success: false,
