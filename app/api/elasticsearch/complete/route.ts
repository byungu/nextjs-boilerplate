...
import { saveProductsToElasticsearch } from "@/lib/elasticsearch-utils";
...
export async function GET( request: Request ) {
    try {
        ...
        console.log(`스크래핑 시작 : 카테고리 = ${category}, 페이지 크기 = ${pageSize}, 최대페이지 = ${maxPages}`);
        ...
        // 4단계 중복 제거
        const uniqueProducts: Product[] = await removeDuplicates(validProducts);
        console.log(`중복 제거 완료: ${uniqueProducts.length} 개`);

        // 5단계 엘라스틱 클라우드 저장
        const saveResult = await saveProductsToElasticsearch(uniqueProducts);
        console.log(`엘라스틱 클라우드 저장 완료 : 저장=${saveResult.savedCount} 개`);
        return NextResponse.json({
            success: true,
            message: '완전한 스크래핑 및 엘라스틱 클라우드` 저장 완료!',
            data: {
