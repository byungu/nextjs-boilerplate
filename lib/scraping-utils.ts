...
export async function fetchProductData(
    ...
    while(hasMorePages) {
        try {
            ...
            console.log(`페이지 ${currentPage} 처리 중 오류 : ${error instanceof Error ? error.message :'알 수 없는 오류'}`);
            currentPage++;
            if(currentPage > maxPages) {
                hasMorePages = false;
                console.log(`최대 페이지 수 (${maxPages})에 도달하여 수집 중단`);
            }
        }
    }
